...
export default function Header() {
  ...
  const linkClass = "px-4 py-2 text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors";
  const iconClass = "p-2 text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors relative inline-flex items-center";
  ...
  return (
    ...
    >
      <nav className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
        {/* Logo avec glassmorphism fort */}
        <Link href="/" className="glass-strong px-5 py-2.5 rounded-2xl shadow-lg">
          <span className="text-xl md:text-2xl font-bold tracking-wider gradient-text">ICE-BI</span>
        </Link>

        {/* Menu desktop avec glassmorphism léger */}
        <div className="hidden lg:flex items-center space-x-1 glass rounded-full px-2 py-1">
        ...
        {/* Icônes droite avec glassmorphism léger */}
        <div className="flex items-center space-x-1 glass rounded-full px-2 py-1">
          ...
          {isLoggedIn ? (
            <div className="relative">
              ...
              {profileOpen && (
                <div className="absolute right-0 top-full mt-3 glass-strong rounded-2xl p-2 min-w-[160px] shadow-xl z-50">
                  <Link href="/profil" onClick={() => setProfileOpen(false)} className="block px-4 py-2.5 text-sm rounded-xl hover:bg-white/10 transition-all">Mon Profil</Link>
                  <Link href="/profil/commandes" onClick={() => setProfileOpen(false)} className="block px-4 py-2.5 text-sm rounded-xl hover:bg-white/10 transition-all">Mes Commandes</Link>
                  {isAdmin && (
                    <Link href="/admin" onClick={() => setProfileOpen(false)} className="block px-4 py-2.5 text-sm rounded-xl hover:bg-white/10 transition-all">⚙️ Dashboard</Link>
                  )}
                  <hr className="my-1 border-[var(--border)]" />
                  <button onClick={() => { logout(); setProfileOpen(false); }} className="block w-full text-left px-4 py-2.5 text-sm text-red-400 rounded-xl hover:bg-red-500/10 transition-all">Déconnexion</button>
                </div>
              )}
            </div>
          ) : (
            ...
            <div className="flex flex-col px-4 py-6 gap-1">
              ...
              {isLoggedIn ? (
                <>
                  ...
                  {isAdmin && <Link href="/admin" onClick={() => setMenuOpen(false)} className="px-4 py-3 rounded-xl">⚙️ Dashboard</Link>}
